...
check_result <- function() {
  if (!exists("cars_per_cycle")) {
    return("🚫 Please create the 'cars_per_cycle' variable and run your code.")
  } else if (identical(cars_per_cycle, 20)) {
    return("🎯 Perfect! 20 cars can pass through in one green cycle. This is a fundamental capacity calculation used in traffic signal timing.")
  } else if (cars_per_cycle > 20) {
    return("🤔 Too many cars! Remember, each car needs 3 seconds to clear the intersection. Try dividing the green time by the time per car.")
  } else if (cars_per_cycle < 20) {
    return("🚗 Not quite enough cars! How many 3-second periods fit in 60 seconds? Think: 60 ÷ 3 = ?")
  } else {
    return("🔢 Make sure your result is a number. Check your calculation.")
  }
}
...
check_result <- function() {
  if (!exists("vc_ratio") || !exists("los_category")) {
    return("🚫 Please create both 'vc_ratio' and 'los_category' variables.")
  } else {
    vc_correct <- round(vc_ratio, 3) == round(1600/1800, 3)
    los_correct <- los_category == "D"
    
    if (vc_correct && los_correct) {
      return("🎯 Excellent! V/C = 0.889 puts this intersection at LOS D. This indicates approaching unstable flow with tolerable delays.")
    } else if (!vc_correct && los_correct) {
      return("🔢 LOS category is correct, but check your V/C calculation: volume ÷ capacity = 1600 ÷ 1800")
    } else if (vc_correct && !los_correct) {
      return("📊 V/C ratio is correct (0.889), but check the LOS thresholds: 0.8-0.9 = Level D")
    } else {
      return("🚦 Both calculations need work. V/C ratio = volume ÷ capacity, then check: ≤0.8=A-C, 0.8-0.9=D, >0.9=E-F")
    }
  }
}
...
check_result <- function() {
  if (!exists("transit_share_of_non_car")) {
    return("🚫 Please create the 'transit_share_of_non_car' variable and run your code.")
  } else if (round(transit_share_of_non_car, 1) == 41.7) {
    return("🚌 Perfect! 41.7% of non-car trips are made by transit. This shows transit's importance in the sustainable transportation mix.")
  } else if (transit_share_of_non_car == 25) {
    return("🤔 That's the overall transit share (25%). But what about just among non-car trips? Hint: transit % ÷ non-car total % × 100")
  } else if (round(transit_share_of_non_car, 0) == 42) {
    return("🚌 Very close! 41.7% of non-car trips are made by transit. Your calculation method is correct!")
  } else {
    return("📊 Remember: First find non-car total (100 - car %), then calculate: (transit %) ÷ (non-car total %) × 100")
  }
}
...
check_result <- function() {
  if (!exists("total_daily_trips") || !exists("pm_peak_trips")) {
    return("🚫 Please create both 'total_daily_trips' and 'pm_peak_trips' variables.")
  } else {
    daily_correct <- total_daily_trips == 1900
    peak_correct <- pm_peak_trips == 285
    
    if (daily_correct && peak_correct) {
      return("🏠 Excellent! 1,900 daily trips with 285 during PM peak. These are key numbers for traffic impact studies.")
    } else if (!daily_correct && peak_correct) {
      return("🏘️ PM peak trips are correct, but check daily trips: units × trip rate = 200 × 9.5")
    } else if (daily_correct && !peak_correct) {
      return("🕔 Daily trips correct, but check PM peak: daily trips × 15% = 1900 × 0.15")
    } else {
      return("📊 Both calculations need work. Daily = units × rate, PM peak = daily × 15%")
    }
  }
}
...
check_result <- function() {
  if (!exists("buses_per_hour") || !exists("hourly_passenger_capacity")) {
    return("🚫 Please create both 'buses_per_hour' and 'hourly_passenger_capacity' variables.")
  } else {
    buses_correct <- buses_per_hour == 5
    capacity_correct <- hourly_passenger_capacity == 150
    
    if (buses_correct && capacity_correct) {
      return("🚍 Perfect! 5 buses per hour with 150 passenger capacity. This helps determine route efficiency and passenger service levels.")
    } else if (!buses_correct && capacity_correct) {
      return("🕒 Passenger capacity is correct, but check buses per hour: 60 minutes ÷ frequency = 60 ÷ 12")
    } else if (buses_correct && !capacity_correct) {
      return("👥 Buses per hour correct, but check capacity: buses × capacity × load factor = 5 × 40 × 0.75")
    } else {
      return("🚌 Both calculations need work. Buses/hour = 60 ÷ frequency, Capacity = buses × seats × load factor")
    }
  }
}
...
check_result <- function() {
  if (!exists("high_traffic") || !exists("count_high_traffic")) {
    return("📊 Make sure to create both 'high_traffic' and 'count_high_traffic' variables.")
  } else {
    ...
    if (filter_correct && count_correct) {
      return("🎯 Perfect! You correctly identified 3 high-traffic locations (Main St, Center Blvd, Highway 101) with AADT > 15,000. This type of analysis helps prioritize infrastructure investments.")
    } else if (!filter_correct && count_correct) {
      return("🔍 Count is correct, but check your filtering condition. You should use: filter(aadt > 15000)")
    } else if (filter_correct && !count_correct) {
      return("📋 Filtering is correct, but check your count calculation. Use nrow() to count the filtered results.")
    } else {
      return("🚗 Check both your filter condition (aadt > 15000) and count method (use nrow()).")
    }
  }
}
...
check_result <- function() {
  if (!exists("safety_analysis")) {
    return("📊 Make sure to create the 'safety_analysis' variable.")
  } else {
    ...
    if (!"accident_rate" %in% names(safety_analysis)) {
      return("📋 Make sure to create the 'accident_rate' column using mutate().")
    } else if (!identical(safety_analysis, expected_result)) {
      unsorted_check <- traffic_data |>
        mutate(accident_rate = (accidents_2023 / aadt) * 1000)
      
      if (identical(safety_analysis, unsorted_check)) {
        return("📈 Accident rate calculation is correct, but you need to arrange by desc(accident_rate) to show highest rates first.")
      } else {
        return("🔢 Check your accident rate calculation: (accidents_2023 / aadt) * 1000")
      }
    } else {
      return("🎯 Excellent! You've calculated accident rates per 1,000 AADT and sorted by highest risk. Park Rd has the highest rate at 0.67 accidents per 1,000 AADT, making it a priority for safety improvements.")
    }
  }
}
...
check_result <- function() {
  if (!exists("transit_efficiency") || !exists("most_efficient")) {
    return("📊 Make sure to create both 'transit_efficiency' and 'most_efficient' variables.")
  } else {
    ...
    if (efficiency_correct && route_correct) {
      return("🎯 Perfect! Route 3 is the most efficient with 114.8 riders per mile. This analysis helps transit agencies optimize route planning and resource allocation.")
    } else if (!efficiency_correct && route_correct) {
      return("🚌 Most efficient route is correct, but check your efficiency calculation: daily_riders / route_length_miles")
    } else if (efficiency_correct && !route_correct) {
      return("📊 Efficiency calculation is correct, but check how you're extracting the most efficient route name using pull(route)")
    } else {
      return("🚍 Check both your efficiency calculation (riders ÷ miles) and route extraction (slice(1) |> pull(route))")
    }
  }
}
...
check_result <- function() {
  if (!exists("bike_plot")) {
    return("📊 Make sure to create the 'bike_plot' variable.")
  } else if (!inherits(bike_plot, "ggplot")) {
    return("📈 Make sure 'bike_plot' is a ggplot object.")
  } else {
    ...
    if (x_correct && y_correct) {
      return("🎯 Excellent visualization! Your plot shows the relationship between distance from downtown and bike share usage. The trend line reveals that stations closer to downtown tend to have higher usage - a key insight for bike share planning.")
    } else if (!x_correct && y_correct) {
      return("📊 Y-axis (daily_trips) is correct, but check your x-axis mapping: use distance_downtown_km")
    } else if (x_correct && !y_correct) {
      return("📊 X-axis (distance_downtown_km) is correct, but check your y-axis mapping: use daily_trips")
    } else {
      return("🚲 Check your aesthetic mappings: x = distance_downtown_km, y = daily_trips")
    }
  }
}
...
check_result <- function() {
  if (!exists("performance_summary")) {
    return("📊 Make sure to create the 'performance_summary' variable.")
  } else {
    expected_result <- traffic_data |>
      mutate(
        daily_volume = aadt,
    ...
    if (!has_correct_cols) {
      ...
      return("🎯 Outstanding! You've created a comprehensive performance dashboard showing volume, safety rates, and congestion levels. This type of integrated analysis is essential for transportation planning and helps identify priority locations for improvements.")
    } else {
      ...
      if (!daily_vol_correct) {
        return("📊 Check daily_volume calculation: should be the same as aadt")
      } else if (!accidents_correct) {
        return("🔢 Check accidents_per_1000 calculation: (accidents_2023 / aadt) * 1000")
      } else if (!congestion_correct) {
        return("🚦 Check congestion_level case_when logic: <3='Low', <=5='Medium', >5='High'")
      } else {
        return("📋 Almost there! Check your mutate and select statements carefully.")
      }
    }
  }
}

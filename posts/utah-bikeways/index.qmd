...
```{r}
# Define paths
path_ut_counties <- file.path(dir_data, "UtahCountyBoundaries.gdb")
...
// 1. Transpose data from column-arrays to row-objects
...
// 3. Prepare data for plotting (Long format for stacked bars)
long_data = miles.flatMap(d => {
  ...
  return types.map(t => ({
    ...
    Type: t,
    Miles: d[t],
    TotalBike: types.reduce((sum, type) => sum + (d[type] || 0), 0),
    AllRoads: d["All Roads"]
  }));
})

// 4. Filter data based on inputs
filtered = long_data.filter(d => {
  const nameMatch = d.City.toLowerCase().includes(search.toLowerCase());
  const mileMatch = d.TotalBike >= minMiles;
  return nameMatch && mileMatch;
})

// 5. Render Plot
Plot.plot({
  marginLeft: 140,
  height: Math.max(400, new Set(filtered.map(d => d.City)).size * 20), // Dynamic height
  style: {fontSize: "12px"},
  x: {label: "Miles", grid: true},
  y: {label: null},
  color: {
  ...
  marks: [
    // Stacked Bars for Bike Infrastructure
    Plot.barX(filtered, {
      x: "Miles",
      y: "City",
      fill: "Type",
      sort: {y: "x", reverse: true}, // Sort cities by total length
      tip: true,
      title: d => `${d.City}\n${d.Type}: ${d.Miles} miles`
    }),
    
    // Black tick mark for Total Road Miles (Comparison)
    Plot.tickX(filtered, {
      x: "AllRoads",
      y: "City",
      stroke: "black",
      strokeWidth: 2,
      title: d => `Total Road Miles: ${d.AllRoads}`
    })
  ]
})
```

...
// 2. Create Inputs (No changes needed here)
...
long_data = miles.flatMap(d => {
  ...
  return types.map(t => ({
    // FIX: Fallback to "Unincorporated/Unknown" if Name is null/NA
...
filtered = long_data.filter(d => {
  // FIX: Ensure d.City exists before calling .toLowerCase()
  const nameMatch = d.City && d.City.toLowerCase().includes(search.toLowerCase());
...
Plot.plot({
  marginLeft: 140,
  height: Math.max(400, new Set(filtered.map(d => d.City)).size * 20),
  style: {fontSize: "12px"},

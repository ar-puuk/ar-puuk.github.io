...
# reticulate::conda_install("metro", c(
#   "numpy", "pandas", "geopandas", "matplotlib", "seaborn",
#   "folium", "pygris", "requests"
# ))
```
...
lscog_acs_cb = gpd.sjoin(lscog_dec, lscog_acs, how="left", predicate="intersects")

# Apply population-weighted interpolation
lscog_acs_cb = (
    lscog_acs_cb.assign(weight=lambda df: df["HH"] / df["HH"].sum())
                .apply(lambda row: row.drop(["geometry", "HH"]) * row["weight"], axis=1)
)

# Convert back to a GeoDataFrame with original block geometries
lscog_acs_cb = gpd.GeoDataFrame(lscog_dec[["GEOID", "geometry"]].join(lscog_acs_cb))
```
:::
